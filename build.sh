...
logInfoMessage " Filtering EC2 instances with tag [$TAG_KEY=$TAG_VALUE] for action [$ACTION]..."
...
if [ -z "$INSTANCE_IDS" ]; then
  logInfoMessage " No matching instances found."
  exit 0
fi
...
for id in $INSTANCE_IDS; do
  ...
  if [ "$ACTION" == "Start" ]; then
    if [ -n "$NEW_INSTANCE_TYPE" ]; then
      logInfoMessage " Stopping instance $id for resizing..."
